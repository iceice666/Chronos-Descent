...
public class Manager : IManager
{
    ...
    {
        if (slot == AbilitySlotType.Unknown) return;
        ...
        _owner.EventBus.Publish(
            EventBus.EventVariant.AbilityChange,
            slot
        );
    }
    ...
    public void ActivateAbility(AbilitySlotType slot)
    {
        ...
        {
            CurrentActiveAbilitySlot = slot;
        }

        _owner.EventBus.Publish(
            EventBus.EventVariant.AbilityActivate,
            ability
        );
    }
    ...
    public BaseAbility GetAbility(AbilitySlotType slot) => _abilitySlots.GetAbility(slot);
    public bool CanActivateAbility(AbilitySlotType slot) => GetAbility(slot)?.CanActivate() ?? false;
...
public class AbilitySlots
{
    ...
    public BaseAbility GetAbility(AbilitySlotType slot)
    {
        return slot switch
        {
            ...
            AbilitySlotType.LifeSave => _lifeSaveAbility,
            _ => null
        };
    }
}
