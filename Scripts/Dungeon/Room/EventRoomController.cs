...
public partial class EventRoomController : Node2D
{
    ...
    /// </summary>
    [Export]
    public Control EventPanel { get; set; }

    /// <summary>
    ///     Labels for choice buttons - allows for customization per event
    /// </summary>
    [Export]
    public string Button1Text { get; set; } = "Accept";
    ...
    private void SelectRandomEvent()
    {
        ...
        _eventTitleLabel.Text =
            $"[center][wave amp=50.0 freq=5.0][color=#add8e6]{_currentEvent.Title}[/color][/wave][/center]";
        ...
        _availableEvents.Add(new EventData
        {
            ...
            OnOption1Selected = () =>
            {
                ...
                if (GD.Randf() < 0.7f)
                {
                    ...
                    foreach (var ability in abilities) ability.ReduceCooldown(ability.Cooldown * 0.5);
